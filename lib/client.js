...
function checkCancelled(signal) {
  if (signal?.aborted) throw new JevError('JEV_E_CANCELLED', 'İstek iptal edildi');
}
...
export class JevClient {
  ...
  async decide({ state, questions, model }, { timeoutMs = DEFAULT_TIMEOUT_MS, maxRetries = DEFAULT_MAX_RETRIES, signal } = {}) {
    checkCancelled(signal);
    if (this.mockMode) {
      let out;
      try { out = await this._mockDecide({ state, questions, model }, signal); }
      catch (e) { checkCancelled(signal); throw e; }
      checkCancelled(signal);
    ...
    for (let attempt = 1; ; attempt++) {
      ...
      try {
        const out = await this._fetchOnce({ state, questions, model }, remaining, signal);
        checkCancelled(signal);
        ...
        if (e instanceof JevError && RETRYABLE_STATUS.has(e.status) && attempt <= maxRetries) {
          ...
          try { await this.sleep(waitMs, signal); }
          catch (sleepError) { checkCancelled(signal); throw sleepError; }
          continue;
        }
  ...
  _validateResponse(source, questions, out) {
    ...
    for (const [id, q] of Object.entries(questions)) {
      ...
      if (a.value === undefined || a.value === null) {
        ...
        if (typeof a.value !== 'string' || !Object.hasOwn(q.criteria || {}, a.value)) errs.push(`${id}: geçersiz seçenek`);
      ...
      if (a.probabilities != null) {
        if (typeof a.probabilities !== 'object') {
          ...
          for (const [k, p] of Object.entries(a.probabilities)) {
            if (!isProbability(p)) errs.push(`${id}.probabilities.${k}: [0,1] dışında`);
          }
        }
      }
    }
  ...
  async probeModels({ timeoutMs = 5000 } = {}) {
    ...
    try {
      ...
      if (e instanceof JevError) throw e;
      throw new JevError('JEV_E_UNAVAILABLE', `GET /v1/models ağ/gövde hatası: ${e?.message || e}`);
    } finally {
      clearTimeout(timer);
    }
  }

  async _fetchOnce({ state, questions, model }, timeoutMs, signal) {
    const controller = new AbortController();
  ...
  async _mockDecide({ state, questions, model }, signal) {
    if (this.cfg.mockDelayMs > 0) await this.sleep(this.cfg.mockDelayMs, signal);
...
export function normalizeAnswers(rawAnswers) {
  ...
  for (const [id, raw] of Object.entries(rawAnswers ?? {})) {
    ...
    // Ham değerleri dönüştürmeden doğrula; bozuk confidence veya noul gizlenmemeli.
    if (raw.confidence != null && !isProbability(raw.confidence)) {
      throw new JevError('JEV_E_BAD_RESPONSE', `${id}: geçersiz confidence`);
    }
    if (Object.hasOwn(raw, 'noul') && !isProbability(raw.noul)) {
      throw new JevError('JEV_E_BAD_RESPONSE', `${id}: geçersiz noul olasılığı`);
    }
    const a = { value: null, probabilities: raw.probabilities ?? null, confidence: raw.confidence ?? null };
    if (raw.choice !== undefined) {
      a.value = raw.choice;
    } else if (raw.score !== undefined) {
      a.value = raw.score;
    } else if (typeof raw.noul === 'number') {
      // Makine tüketicileri de eşik karşılaştırır: olasılık hassasiyetini koru.
      const p = raw.noul;
      a.value = p >= 0.5 ? 'yes' : 'no';
      a.probabilities = { yes: p, no: 1 - p };
    } else {
